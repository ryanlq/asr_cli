...
Qwen3-ASR / SenseVoice / Paraformer 有**硬性上下文上限**(约 20–35 秒)。长音频会被 **silero VAD** 自动按静音切分成 ≤25s 的片段分别识别,再拼接;输出 SRT/JSON 时每段带时间戳。(Qwen3-ASR 的模型目录自带 `silero_vad.onnx`;其它模型首次用 VAD 会自动拉取一个共享的 VAD,~2MB。)
